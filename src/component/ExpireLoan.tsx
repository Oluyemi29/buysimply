...
const ExpireLoanComponent = () => {
  const { userData, LogOut, GetAllExpiredLoan,DeleteLoan } = userAuth();
  ...
  useEffect(() => {
    const GetAllExpiredData = async () => {
      const response = (await GetAllExpiredLoan()) as LoanDataProps[];
      setAllLoans(response as LoanDataProps[]);
    };
    GetAllExpiredData();
  }, [GetAllExpiredLoan, userRole]);
...
export default ExpireLoanComponent;
