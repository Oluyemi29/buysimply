...
const EachUserComponent = () => {
  ...
  const { userData, LogOut, GetSingleUserLoan, DeleteLoan } = userAuth();
  ...
  useEffect(() => {
    const GetSingleUserLoanData = async () => {
      const response = await GetSingleUserLoan(email as string);
      setAllLoans(response as LoanDataProps[]);
    };
    GetSingleUserLoanData();
  }, [GetSingleUserLoan, userRole, email]);
